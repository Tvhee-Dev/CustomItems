    ...
    public static IconMenu getMenu(int page)
    {
        ...
        IconMenu menu = new IconMenu(ChatUtils.format(Messages.menu), 54, new IconMenu.OptionClickEventHandler()
        {
            ...
            public void onOptionClick(IconMenu.OptionClickEvent e)
            {
                ...
                if(position < 45)
                {
                    if(e.getClick().isLeftClick())
                    {
                        ...
                        if(itemsByName[position + (page - 1) * 45] != null)
                        {
                            ...
                            e.openNewMenu(EditMenu.getMenu(name, page), player);
                            return;
                        }
                        else
                        {
                ...
                {
                    player.sendMessage(ChatUtils.format(Messages.igprefix + Messages.typeInNewItemName));
                    ItemUtil.putInventory(player, position, page, "CustomItemsMainMenu");
                    e.setWillClose(true);
                ...
                {
                    player.sendMessage(ChatUtils.format(Messages.igprefix + Messages.typeInOldItemName));
                    ItemUtil.putInventory(player, position, page, "CustomItemsMainMenu");
                    e.setWillClose(true);
                    e.setWillDestroy(true);
                    return;
        ...
        if(plugin.getConfig().contains(ConfigUtil.configurationSection))
        {
            for(int i = 0; i < 45; i++)
            {
                ...
                {
                    String name = itemsByName[i + (page - 1) * 45].toString();
                    String displayName = ConfigUtil.getItemDisplayName(name);
                    ...
                    menu.setOption(i, item, displayName, lore);
                }
                else
                {
                    break;
        ...
        menu.setOption(45, new ItemStack(Material.PAPER, 1), ChatUtils.format(Messages.mainMenuCreate), null);
        menu.setOption(46, new ItemStack(Material.BOOK, 1), ChatUtils.format(Messages.mainMenuDuplicate), null);
